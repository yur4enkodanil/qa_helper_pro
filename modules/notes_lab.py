...
def render_notes_lab():
    ...
    st.subheader("📝 Лаборатория заметок")
    ...
    with st.expander("⚙️ Управление шаблонами"):
        st.write("Здесь можно изменить текст кнопок или добавить новые.")
        
        # Редактирование существующих
        updated_templates = {}
        for label, text in templates.items():
            c_name, c_text, c_del = st.columns([1, 2, 0.5])
            new_label = c_name.text_input("Название кнопки", value=label, key=f"lab_{label}")
            new_text = c_text.text_area("Текст шаблона", value=text, key=f"txt_{label}", height=68)
            if not c_del.button("🗑️", key=f"del_{label}"):
                updated_templates[new_label] = new_text
        
        st.divider()
        # Добавление нового
        st.write("**Добавить новый шаблон:**")
        add_col1, add_col2, add_col3 = st.columns([1, 2, 0.5])
        add_label = add_col1.text_input("Название", key="add_label", placeholder="Напр: API")
        add_text = add_col2.text_area("Текст", key="add_text", placeholder="Endpoint: ...")
        ...
        if st.button("💾 Сохранить все изменения шаблонов", use_container_width=True):
            save_templates(updated_templates)
            if logger: logger("Заметки: Конфигурация шаблонов обновлена")
            st.rerun()
    ...
    with col_list:
        st.write("### 📂 Списки")
        current_file = st.radio("Файл:", existing_files, label_visibility="collapsed")
        new_name = st.text_input("Новый список:", key="new_file_input")
        if st.button("➕ Создать файл", use_container_width=True):
            if new_name:
                with open(os.path.join(NOTES_DIR, f"{new_name.strip()}.txt"), "w", encoding="utf-8") as f:
                    f.write("")
                st.rerun()
    ...
    with col_edit:
        ...
        # Редактор
        st.text_area(f"Правка: {current_file}", height=400, key=state_key)
        
        # Сохранение фона
        with open(file_path, "w", encoding="utf-8") as f:
            f.write(st.session_state[state_key])

        if st.button(f"🗑️ Удалить {current_file}", type="secondary"):
